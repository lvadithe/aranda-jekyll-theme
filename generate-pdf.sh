...
# List all files ending with .md in the _pages directory and all files with any extension in the assets directory, sorting by numerical order starting from second field separated by "/"
FILES=$(
  find _pages assets -type f \( -name "*.md" -o -not -name ".*" \) |
  sed -e 's#/en\?/##g' -e 's#/en\?##g' |
  sort -t '/' -k 2n
)
...
for line in $FILES; do
  ...
  if [[ $CHAPTER != "Inicio" ]]; then
    # Write chapter if not exists
    if [ $EXISTS_CHAPTER -eq 0 ]; then
      echo $CHAPTER
